...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    // Note: Later we will update this query to only fetch reports created by THIS user.
    // For now, it shows recent reports.
    ...
    Alert.alert('Sign Out', 'Are you sure you want to sign out?', [
      ...
      { 
        ...
        onPress: async () => {
          try {
            await signOut(auth);
            // App.js will detect this and automatically send you back to the Welcome Screen!
          } catch (error) {
...
const styles = StyleSheet.create({
  ...
  statNumber: { fontSize: 26, fontWeight: '800', color: '#154212' },
